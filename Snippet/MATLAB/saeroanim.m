...
%
   ...
   if ~strcmpi(get(h_anim,'windowStyle'),'docked')
       set(h_anim,'resize','off','position',[100 100 900 700]);
   end
...
%   
	...
	set(handle.target,'faceColor',[1 0 1], ...
                  'edgeColor',[0 0 0]);
...
%
   [xm,ym,zm] = miss_shape;
   handle.craft = patch(xm,ym,zm,[1 1 0]);
...
%	count=1;	     % Counter for number of individual patches
    ...
    TIP_X       = 1.0;
	len         = 7.0;              % Total Length (no units)
	radius		= 0.2;              % Radius of body
 	s_fore	 	= 0.1;              % Start of main body (w.r.t. nose)
   	thr_len 	= 0.2;              % Length of Motor exit
 	rad_throt	= 0.1;              % radius of motor exit
...
%
	...
	for i = 1:num
	   xcraft{i}=[0 0 0+l_fore 0+l_fore ] + s_fore - TIP_X;
...
%
	...
	for i = 1:num
	  xcraft{count}=[len-thr_len len-thr_len len len] - TIP_X;
...
%
	for i = 1:num
	  xcraft{count}=[s_fore s_fore 0 s_fore] - TIP_X;
...
%
	...
	xcraft{count}  = M_LE_X +                  [0.0, (cR-cT)/2, cR-(cR-cT)/2, cR];
	ycraft{count}  =          cosd(dihedral) * [0.0, span/2, span/2, 0.0];
	zcraft{count}  = M_LE_Z + sind(dihedral) * [0.0, span/2, span/2, 0.0];
...
%
    ...
    cR     = 1.0;
    cT     = 0.8;
	xcraft{count} = T_LE_X + [0.0, cR-cT, cR, cR];
	ycraft{count} =          [0.0, span/2, span/2, 0.0];
	zcraft{count} = T_LE_Z + [0, 0, 0, 0];
  	xcraft{count+1} = xcraft{count};
	ycraft{count+1} =-ycraft{count};
	zcraft{count+1} = zcraft{count};
   	count = count + 2;
    ...
    cT     = 0.8;
	xcraft{count} = T_LE_X + [0.0, cR-cT, cR, cR];
	ycraft{count} = T_LE_Z + [0, 0, 0, 0];
	zcraft{count} =          [0.0, span/2, span/2, 0.0];
   	count = count + 1;
...
%    rollD = 45;
%    DCM = [1 0 0;0 cosd(rollD) sind(rollD);0 -sind(rollD) cosd(rollD)];
%    for i =1:4
%       dum = [x(i,:);y(i,:);z(i,:)];
%       dum = DCM*dum;
%       x(i,:)=dum(1,:);
